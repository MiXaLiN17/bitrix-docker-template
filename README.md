...
**Стек:** Nginx 1.29 · PHP-FPM 8.1–8.4 · MySQL 8.0–8.4 или MariaDB 10.11/11.4 · Sphinx 2.2.11 · memcached 1.6
...
| Команда | Описание |
|---|---|
| `make mysql` | Подключиться к БД (root) |
| `make db-connect` | Подключиться к БД (пользователь из .env) |
| `make db-reset` | Пересоздать volume БД (все данные будут удалены!) |

### Движок и версия БД

Задаются при установке или переменными `DB_ENGINE` (`mysql` / `mariadb`) и `DB_VERSION` в `.env`.
Конфиг движка берётся из `.docker/<движок>/conf/my.cnf`, путь подставляется в контекст сборки
сервиса `db`.

| `DB_ENGINE` | `DB_VERSION` | Статус |
|---|---|---|
| `mysql` | `8.4` | LTS, рекомендуется Битриксом, поддержка до 2032 — значение по умолчанию |
| `mysql` | `8.3` / `8.2` / `8.1` | Innovation-релизы, сняты с поддержки; нужны, только чтобы повторить конфигурацию старого прода |
| `mysql` | `8.0` | LTS, минимальная версия для Битрикса |
| `mariadb` | `11.4` | LTS, поддержка до 2029 — значение по умолчанию для этого движка |
| `mariadb` | `10.11` | LTS, поддержка до 2028 |

Битрикс требует минимум MySQL 8.0, рекомендованная версия — 8.4 и выше.

#### Какой движок выбрать

По умолчанию — MySQL. MariaDB нужна, когда **дамп снят с MariaDB 11.4+**: с этой версии там появились
коллации `*_uca1400_*`, и MySQL их не знает — импорт падает с `Unknown collation`, причём падает
посреди процесса, уже накатив часть таблиц:

```
MySQL query error! Unknown collation: 'utf8mb3_uca1400_ai_ci'
```

Обратная сторона слабее: MariaDB 11.4 понимает коллации MySQL 8 (`utf8mb4_0900_ai_ci` и др.), так что
MySQL-дамп в неё зайдёт. Но перекладывать на это всю работу не стоит — Битрикс рекомендует MySQL.

Посмотреть, какие коллации несёт дамп, до импорта:

```bash
grep -ohiE 'COLLATE[= ]+`?[a-z0-9_]+' dump.sql | grep -oiE '[a-z0-9]+_[a-z0-9_]+$' | sort -u
```

Если в выводе есть `uca1400` — дамп из MariaDB. Альтернатива смене движка — заменить коллацию
прямо в дампе (`sed -i 's/_uca1400_ai_ci/_general_ci/g' dump.sql`), но менять её у таблиц с русским
текстом означает менять и правила сортировки.

**Менять движок или версию на уже установленном проекте нельзя:** каталог данных несовместим и между
движками, и между версиями одного движка, особенно при откате назад. Смена требует `make db-reset`
(данные удаляются) либо дампа и повторного импорта.
...
| `DB_ENGINE` | Движок БД: `mysql` или `mariadb` |
| `DB_VERSION` | Версия движка (mysql: 8.0 / 8.1 / 8.2 / 8.3 / 8.4, mariadb: 10.11 / 11.4) |
